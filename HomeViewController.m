...
@interface HomeViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,NSURLSessionDelegate,HomeWidDelegate,passDataAfterParsing,viewAllDelegate,UISearchBarDelegate,UITextFieldDelegate>
{
...
- (void)viewDidLoad
{
    ...
    self.tblBanner.frame = CGRectMake(0, 0, self.view.frame.size.width, 200);
 ...
 //   self.scrollViewObj.backgroundColor=[UIColor greenColor];
    [_bannerCollectionView registerClass:[UICollectionViewCell class] forCellWithReuseIdentifier:@"cellIdentifier"];
    appDelObj.fromWhere=@"";
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    self.categoryView.frame=CGRectMake(0, self.tblBanner.frame.origin.y+ self.tblBanner.frame.size.height+10, self.categoryView.frame.size.width, self.categoryView.frame.size.height);

    appDelObj.dealBundle=@"";
    self.btnViewMoewCat.clipsToBounds=YES;
    self.btnViewMoewCat.layer.cornerRadius=3;
    [[self.btnViewMoewCat layer] setBorderWidth:1.0f];
    [[self.btnViewMoewCat layer] setBorderColor:[appDelObj.BlueColor CGColor]];
...
//        [self.tblBanner needsUpdateConstraints];
   ...
   // self.categoryView.center = CGPointMake(self.categoryView.frame.size.width  / 2,
                           //          self.categoryView.frame.size.height / 2);
...
-(void)getLanguage
{
    ...
    [webServiceObj getDataFromService:[NSURLRequest requestWithURL:url]];
//    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
//    NSURLResponse * response;
//
////    NSURL *blogURL = [NSURL URLWithString:urlStr];
...
-(void)finishedParsingDictionary:(NSDictionary *)dictionary
{
    ...
    NSString *fb=[NSString stringWithFormat:@"%@",[dictionary objectForKey:@"facebook_enable_login"]];
    ...
    {
        fb=[NSString stringWithFormat:@"%@",[dictionary objectForKey:@"facebook_enable_login"]];
    }
        ...
        
    {
        ...
        {
            if ([catOrBanner isEqualToString:@"Language"])
            {
                lanAry=[dictionary objectForKey:@"result"];
                [[NSUserDefaults standardUserDefaults]setObject:[dictionary objectForKey:@"contactEmail"] forKey:@"CustomerEmail"];
                [[NSUserDefaults standardUserDefaults]setObject:[dictionary objectForKey:@"contactPhone"] forKey:@"CustomerPhone"];
            ...
            else if ([catOrBanner isEqualToString:@"Banner"])
            {
                ...
                if (bannerAry.count==0)
                {
                    ...
                    if(height==0)
                    {
                        self.tblCat.frame=CGRectMake(self.tblCat.frame.origin.x, self.categoryView.frame.size.height+self.categoryView.frame.origin.y-10, self.tblCat.frame.size.width, height+100);
...
//                    [self.tblCat needsUpdateConstraints];
                    ...
                    self.scrollViewObj.contentSize=CGSizeMake(0, self.tblCat.frame.origin.y+self.tblCat.frame.size.height);
                }
            ...
            else if ([catOrBanner isEqualToString:@"Add"])
            {
                ...
                if ([freeproducts isEqualToString:@"Yes"])
                {
                    if(appDelObj.isArabic==YES )
                    {
                        ...
                        listDetail.imgUrl=appDelObj.widgetImgURL;
                ...
                else
                {
                    ...
                    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"" message:@"Item added to cart." preferredStyle:UIAlertControllerStyleAlert];
                    [alertController addAction:[UIAlertAction actionWithTitle:ok style:UIAlertActionStyleDefault handler:nil]];
                    [self presentViewController:alertController animated:YES completion:nil];
...
//                    [self.tblCat needsUpdateConstraints];
                ...
                else
                {
                    _tblBanner.alpha=1;
                    _page.alpha=1;
                  //  self.tblBanner.frame = CGRectMake(-50, 0, self.view.frame.size.width+300, 200);
...
//                    self.carh.constant = self.tblCat.frame.size.height;
//                    [self.tblCat needsUpdateConstraints];
//                    self.bannerH.constant = self.tblBanner.frame.size.height;
//
//                    [self.tblBanner needsUpdateConstraints];
//                    self.bannerw.constant = self.tblBanner.frame.size.width;
//
//                    [self.tblBanner needsUpdateConstraints];
//                    [self.page needsUpdateConstraints];
                   self.tblCat.frame=CGRectMake(self.tblCat.frame.origin.x, self.categoryView.frame.origin.y+self.categoryView.frame.size.height-10, self.tblCat.frame.size.width, height+100);
                    self.scrollViewObj.contentSize=CGSizeMake(0, self.tblCat.frame.origin.y+self.tblCat.frame.size.height);
                }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView==self.tblCat)
    {
        if (widgetAry.count==0)
        {
            ...
            if (indexPath.section==widgetAry.count-1)
            {
                  return 185+100;
            }
            ...
            if (arr.count!=0)
            {
                ...
                //if (largeArray.count!=0&&indexPath.row==1)//[[[[widgetAry objectAtIndex:indexPath.section]valueForKey:@"listBannerWidget"]objectAtIndex:indexPath.row]valueForKey:@"SELF.bannerSize"]isEqualToString:@"Large"])
                {
                    ...
                    catCell.btnB2.alpha=0;
                        ...
                        [catCell.imgStatic sd_setImageWithURL:[NSURL URLWithString:urlIMG] placeholderImage:[UIImage imageNamed:@"placeholder1.png"]];
...
//                    }
                    if (x<smallArr.count)
                    {
                        ...
                        [catCell.btnB2 sd_setBackgroundImageWithURL:[NSURL URLWithString:urlIMG1] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"placeholder1.png"]];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (collectionView==self.bannerCollectionView) {
        return CGSizeMake(self.view.frame.size.width, self.bannerCollectionView.frame.size.height);
    }
...
cell.itemPrice.attributedText=string;
    ...
    double  outStock;
    if ([[[searchArray objectAtIndex:indexPath.row ]  valueForKey:@"itemOptionQuantitySum"]isKindOfClass:[NSNull class]]) {
        outStock=1;
    }
    else
    {
        outStock=[[[searchArray objectAtIndex:indexPath.row ]  valueForKey:@"itemOptionQuantitySum"]doubleValue];
    }
    if (outStock<1) {
        if (appDelObj.isArabic) {
